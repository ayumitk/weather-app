    ...
    <nav class="container flex flex-col sm:flex-row items-center gap-4 py-6">
      <RouterLink :to="{ name: 'home' }">
        <div class="flex items-center gap-3">
          <i class="fa-solid fa-sun text-2xl text-indigo-600"></i>
          <p class="text-2xl text-gray-900">The Local Weather</p>
        </div>
      </RouterLink>

      <div class="flex gap-3 flex-1 justify-end">
        <i
          class="fa-solid fa-circle-info text-xl duration-150 cursor-pointer hover:text-indigo-600"
          @click="toggleModal"
        />
        <i
          class="fa-solid fa-plus text-xl duration-150 cursor-pointer hover:text-indigo-600"
          @click="addCity"
          v-if="route.query"
        />
      </div>
...
const modalActive = ref<Boolean | null>(null);
const toggleModal = () => {
